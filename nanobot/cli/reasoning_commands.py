...
@reasoning_app.command("deterministic-logic")
def toggle_deterministic_logic(action: str | None = typer.Argument(None)) -> None:
    """Toggle deterministic logic (LogicMemory) on/off."""
    if not toggle_feature("deterministic_logic", state, "deterministic_logic_enabled", action or "interactive"):
        raise typer.Exit(code=1)


@reasoning_app.command("memory-v2")
